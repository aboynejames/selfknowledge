...
selfLogic.prototype.frameworklogic = function(intentionin) {
	...
	// links relationships between knowledge words
		...
		case "sync":
			...
			PouchDB.replicate('http://localhost:5984/swimknowledge/', 'selfengine', function(err, response) {
			//PouchDB.replicate('selfengine', 'http://localhost:5984/swimknowledge/' , function(err, response) {	
...
console.log('sync delete');
		...
		break;
			...
			case "twitterin":
			window.open(liveSettings.cloudIP+ "/auth/twitter3", "_self");
		...
		data: cloudready,
			...
			success: function( saveback ){						
				...
				if(serverdatain.save ==  "passedrecord")
				{
					...
					else if(cloudsave.tooltemplate == "Worldrecord-template")
					{
